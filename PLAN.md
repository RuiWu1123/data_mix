...
## One-Dial Act II State

Act II terminates `PARTIAL`: Q1 is `inconclusive` because the frozen protocol has `3` interior scenarios, `0` interior generators, and no full-rank strictly positive public design; Q2-Q5 are all `inconclusive_by_Q1`. Across all `5` questions, real outcome tables read and submitted downstream test jobs are `0/0`, so the H015/H016 descriptive result is neither upgraded nor contradicted. Every count and state is produced by `scripts/audit_onedial_q1_design.py` and `scripts/resolve_onedial_dependencies.py`; inputs `protocol_onedial.json`, the official Olmix ratio tables, and `/work1/ruixiangtang/rw761/data_mix_artifacts/ONEDIAL/Q1/design_audit.json`; commands `sbatch slurm/q1_design_audit.sbatch` and `sbatch slurm/resolve_onedial_dependencies.sbatch`; outputs `/work1/ruixiangtang/rw761/data_mix_artifacts/ONEDIAL/Q1/design_audit.json` and `/work1/ruixiangtang/rw761/data_mix_artifacts/ONEDIAL/dependency_resolution.json`.

## ONEDIAL-V2 Act II State

V2 also terminates `PARTIAL`, now for a distinct detector-resolution defect: `999` permutations imply minimum p-value `0.001`, which exceeds the first Holm threshold in `3` of the `4` registered designs. Q1 is `inconclusive`; Q2-Q5 are `inconclusive_by_Q1`; selected pipelines, synthetic realizations, real outcome tables, and downstream test jobs are all `0`. Every count and state is produced by `scripts/audit_onedial_q1_holm_resolution.py` and `scripts/resolve_onedial_v2_dependencies.py`; inputs `protocol_onedial.json` and `/work1/ruixiangtang/rw761/data_mix_artifacts/ONEDIAL_V2/Q1/holm_resolution_audit.json`; commands `sbatch slurm/q1_holm_resolution_audit.sbatch` and `sbatch slurm/resolve_onedial_v2_dependencies.sbatch`; outputs `/work1/ruixiangtang/rw761/data_mix_artifacts/ONEDIAL_V2/Q1/holm_resolution_audit.json` and `/work1/ruixiangtang/rw761/data_mix_artifacts/ONEDIAL_V2/dependency_resolution.json`.
